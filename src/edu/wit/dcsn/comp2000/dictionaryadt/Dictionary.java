 ...
 * 
 * <p>TODO Make sure you follow all instructions in TODO comments then delete the comments.
 */
...
public class Dictionary<K, V> implements DictionaryInterface<K, V> {
	...
	/*
	 * when the load factor exceeds the threshold, the table must be resized - could
	 * be zero which will force the table to be resized after every add()
	 */
	private float lambda;
	private float lambdaThreshold;

	/*
	 * true: table size will be set to the next prime (if not already prime) false:
	 * table size will be used as is (for initial allocation and when doubled)
	 */
	private boolean forcePrimeTableSize;

	/*
	 * true: prevent enlargeHashTable() from being invoked while it's already
	 * executing false: enlargeHashTable() will execute if invoked (the load factor
	 ...
	 * 
	 * @param initialTableSize the initial number of buckets - if not prime,
	 *                         increased to the next larger prime number - set to
	 *                         the default configuration value if less than or equal
	 *                         to 0
	 */
	 ...
	 * 
	 * @param initialTableSize       initial number of buckets - if not prime,
	 *                               increased to the next larger prime number - set
	 *                               to the default configuration value if less than
	 *                               or equal to 0
	 * @param initialLambdaThreshold a positive, non-zero value - when the load
	 *                               factor exceeds this value, the table will be
	 *                               resized - set to the default configuration
	 *                               value if less than or equal to 0
	 */
	 ...
	 * 
	 * @param initialTableSize           initial number of buckets - if not prime,
	 *                                   increased to the next larger prime number
	 *                                   (if prime table size enforced) otherwise
	 *                                   used as-is - set to the default
	 *                                   configuration value if less than or equal
	 *                                   to 0
	 * @param initialLambdaThreshold     a positive, non-zero value - when the load
	 *                                   factor exceeds this value, the table will
	 *                                   be resized - set to the default
	 *                                   configuration value if less than or equal
	 *                                   to 0
	 * @param initialForcePrimeTableSize if true, table size (number of buckets)
	 *                                   will always be increased to the next prime
	 *                                   number (if not already prime) upon
	 *                                   instantiation and resizing - if false,
	 *                                   table size is used as-is (initialTableSize
	 *                                   and new table size when resized)
	 */
	...
	public Dictionary(int initialTableSize, float initialLambdaThreshold, boolean initialForcePrimeTableSize) {
		...
		MetricsLogger.logInitialMetrics(bucketsInUse, lambdaThreshold, forcePrimeTableSize);
	 ...
	 * 
	 * @see
	 * edu.wit.dcsn.comp2000.dictionaryadt.DictionaryInterface#add(java.lang.Object,
	 * java.lang.Object)
	 */
	@Override
	public V add(K key, V value) {
		if (hashTable[entryCount].add(new TableEntry(key, value))) {
			entryCount++;
			return value;
		}
		return null;
	 ...
	 * 
	 * @see
	 * edu.wit.dcsn.comp2000.dictionaryadt.DictionaryInterface#contains(java.lang.
	 * Object)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * edu.wit.dcsn.comp2000.dictionaryadt.DictionaryInterface#getValue(java.lang.
	 * Object)
	 */
	@Override
	public V getValue(K key) {
		index = hashTable[1].hashCode();
	...
	/**
	 * Increases the size of the hash table to at least twice its old size (to the
	 * next prime number if necessary). This method must rehash the table entries.
	 */
...
//											   forcePrimeTableSize );
	...
	/**
	 * Test a value for prime-ness.\ [based on isPrime() in HashedDictionary.java by
	 * Carrano and Henry]
	 * 
	...
	/**
	 * Get the next prime number greater than or equal to the starting value [based
	 * on getNextPrime() in HashedDictionary.java by Carrano and Henry]
	 * 
	...
	/**
	 * Ensure the candidate table size (number of buckets) is positive, non-zero,
	 * and prime (if necessary).
	 * 
	 * @param candidateTableSize starting value for consideration for table size
	 ...
	 * 
	 * @param candidateThreshold value for consideration for load factor threshold
...
//									   forcePrimeTableSize );
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.wit.dcsn.comp2000.dictionaryadt.DictionaryInterface#
	 ...
	 * 
	 * @see
	 * edu.wit.dcsn.comp2000.dictionaryadt.DictionaryInterface#getPrimeTableSize()
	 */
	@Override
	 ...
	 * 
	 * @see
	 * edu.wit.dcsn.comp2000.dictionaryadt.DictionaryInterface#setPrimeTableSize(
	 * boolean)
	 */
	@Override
	...
	private class TableEntry {
		...
		public boolean equals(Object otherObject) {
			...
			// WARNING: Must search with a TableEntry rather than the key because LinkedList
			// uses
			// the key's class' equals() method, not the element's!!! So, for now,
			// the key handling portion of this method will not execute
			K otherKey;
			if (otherObject.getClass().equals(this.getClass())) { // the parameter is a TableEntry - get the key from it
				otherKey = ((TableEntry) otherObject).key;
			} else if (otherObject.getClass().equals(this.key.getClass())) { // the parameter is a key - use it
